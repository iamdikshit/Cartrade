...
export async function GET(
  ...
  try {
    ...
    const query = isObjectId ? { _id: id } : { $or: [{ slug: id }, { carId: id }] }

    const car: (ICar & { _id: mongoose.Types.ObjectId }) | null = await Car.findOne(query)

    if (!car) return NextResponse.json({ error: 'Car not found' }, { status: 404 })

    Car.findByIdAndUpdate(car._id, { $inc: { views: 1 } }).exec().catch(() => {})

    return NextResponse.json({ success: true, car: JSON.parse(JSON.stringify(car)) })
  } catch {
...
async function updateCar(
  ...
  if (!request.user.permissions.includes('cars:edit') && request.user.role !== 'root') {
  ...
  try {
    ...
    if (!car) return NextResponse.json({ error: 'Car not found' }, { status: 404 })
    return NextResponse.json({ success: true, car, message: 'Car updated successfully' })
  } catch (error) {
    const msg = error instanceof Error ? error.message : 'Unknown error'
    return NextResponse.json({ error: 'Failed to update car', details: msg }, { status: 500 })
  }
}
...
async function deleteCar(
  ...
  if (!request.user.permissions.includes('cars:delete') && request.user.role !== 'root') {
    return NextResponse.json({ error: 'Permission denied' }, { status: 403 })
  }
  try {
    await connectDB()
